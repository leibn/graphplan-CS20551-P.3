    ...
    """
      ...
      We will define limitations of action to be the union of:
        ...
        And then we will check for each of the limits whether the proposition prop
         is a negative effect of the havey action
         """

    a1Limitations = a1.getPre() + a1.getAdd()
    for p in a1Limitations:
      if a2.isNegEffect(p):
          return False
    a2Limitations = a2.getPre() + a2.getAdd()
    for p in a2Limitations:
        if a1.isNegEffect(p):
            return False
    return True
